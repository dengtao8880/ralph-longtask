...
export function runAcceptanceChecks(story, validationConfig = {}, options = {}) {
  ...
  const needsBrowserVerification = criteria.some(
    (criterion) => normalizeCriterion(criterion) === 'verify in browser using dev-browser skill',
  );
  ...
  if (needsBrowserVerification) {
    if (!acceptanceCommands.browser) {
      return {
        valid: false,
        reason: 'acceptance-check-unconfigured',
        check: 'browser',
      };
    }
    checks.push({ name: 'browser', command: acceptanceCommands.browser });
  }
