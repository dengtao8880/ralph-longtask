...
function printOrchestrationResult(result) {
  if (result.status === 'blocked') {
    ...
    if (result.reason === 'missing') {
      ...
      console.log(chalk.dim('Next step: finish the OpenSpec change so proposal.md, specs/, design.md, and tasks.md all exist, then resume the pipeline.'));
    } else if (result.reason === 'openspec_required') {
      console.log(chalk.dim('OpenSpec is required for this pipeline.'));
      console.log(chalk.dim('Next step: install the OpenSpec CLI or open an OpenSpec-enabled agent, then run `/opsx:explore` or `/opsx:propose` before resuming.'));
      ...
      console.log(chalk.dim('Next step: if the request is still fuzzy, run `/opsx:explore` first.'));
      ...
      console.log(chalk.dim('Next step: hand these OpenSpec documents to Superpowers, review them, and produce tasks/prd-*.md before resuming.'));
      ...
      console.log(chalk.dim('Next step: run the ralph skill to convert the approved PRD markdown into prd.json, then resume the pipeline.'));
      ...
      if (result.metadata?.archiveMode === 'manual-opsx-archive-required') {
        console.log(chalk.dim('Next step: run `/opsx:archive` in your OpenSpec-enabled agent to archive the completed change.'));
      } else {
        console.log(chalk.dim('Next step: install the OpenSpec CLI or run `/opsx:archive` in an OpenSpec-enabled agent to finish the pipeline.'));
      }
