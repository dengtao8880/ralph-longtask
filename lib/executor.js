 ...
 * @param {object} config - full config object { permissionsMode, claude: { maxTurns } }
...
export function executeSession(prompt, config, options = {}) {
  const {
    ...
    story,
    spawnProcess = spawn,
    resolveClaudeBinary = findClaudeBinary,
    resolveClaudeScriptPath = findClaudeScriptPath,
  ...
  return new Promise((resolve, reject) => {
    ...
    if (IS_WINDOWS) {
      ...
      const scriptPath = resolveClaudeScriptPath();
      if (scriptPath) {
        child = spawnProcess(process.execPath, [scriptPath, ...args], {
          stdio: ['pipe', 'pipe', 'pipe'],
        });
      } else {
        // shell: true needed for .cmd files; quote path to handle spaces
        child = spawnProcess(`"${claudeBin}"`, args, {
          stdio: ['pipe', 'pipe', 'pipe'],
          shell: true,
        });
      }
    ...
    child.stdout?.on('data', (chunk) => {
      ...
      capturedStdout += text;
      if (!completionSignaled && text.includes('<promise>COMPLETE</promise>')) {
        completionSignaled = true;
      }
      stdoutDestination.write(chunk);
    });

    child.stderr?.on('data', (chunk) => {
      capturedStderr += chunk.toString();
      stderrDestination.write(chunk);
    });
