...
import { validateBudgetConfiguration } from './budget.js';
...
function applyEnvOverrides(config) {
  ...
  const nestedEnvMap = {
    BUDGET_MAX_TOTAL_TOKENS: ['budget', 'maxTotalTokens'],
    BUDGET_MAX_TOTAL_COST_USD: ['budget', 'maxTotalCostUsd'],
    BUDGET_CHARS_PER_TOKEN: ['budget', 'charsPerToken'],
    BUDGET_INPUT_COST_PER_1K_TOKENS_USD: ['budget', 'inputCostPer1kTokensUsd'],
    BUDGET_OUTPUT_COST_PER_1K_TOKENS_USD: ['budget', 'outputCostPer1kTokensUsd'],
    ...
    VALIDATION_ACCEPTANCE_COMMANDS_BROWSER: ['validation', 'acceptanceCommands', 'browser'],
  };
...
function validateConfig(config) {
  const numericFields = [
    ...
    ['budget.maxTotalTokens', config.budget.maxTotalTokens, true],
    ['budget.maxTotalCostUsd', config.budget.maxTotalCostUsd, true],
    ['budget.charsPerToken', config.budget.charsPerToken, false],
    ['budget.inputCostPer1kTokensUsd', config.budget.inputCostPer1kTokensUsd, true],
    ['budget.outputCostPer1kTokensUsd', config.budget.outputCostPer1kTokensUsd, true],
  ];

  for (const [name, value, allowZero] of numericFields) {
    const requireInteger = name !== 'budget.maxTotalCostUsd'
      && name !== 'budget.inputCostPer1kTokensUsd'
      && name !== 'budget.outputCostPer1kTokensUsd';
    ...
    if (!allowZero && value <= 0) {
      throw new Error(`${name} must be a ${requireInteger ? 'positive integer' : 'positive number'}`);
    }
    if (allowZero && value < 0) {
      throw new Error(`${name} must be a ${requireInteger ? 'non-negative integer' : 'non-negative number'}`);
    }
  }
